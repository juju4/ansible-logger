...
describe file('/var/log/syslog'), :if => os[:family] == 'debian' || os[:family] == 'ubuntu' do
  ...
  its(:content) { should match /kolide\[[0-9]+\]: {"address":"0.0.0.0:8412","msg":"listening","transport":/ }
...
describe file('/var/log/messages'), :if => os[:family] == 'redhat' && os[:release] == '7' do
  ...
  its(:content) { should match /kolide: {"address":"0.0.0.0:8412","msg":"listening","transport":/ }
...
describe command('curl -vk https://localhost:8412/healthz') do
  its(:stdout) { should match /^$/ }
#  its(:stderr) { should match /HTTP\/1.1 200 OK/ }
  its(:exit_status) { should eq 0 }
end
describe command('curl -vk -X POST https://localhost:8412/api/v1/osquery/enroll') do
  its(:stdout) { should match /Unknown Error/ }
#  its(:stderr) { should match /HTTP\/1.1 500 / }
#  its(:exit_status) { should_not eq 0 }
end
describe command('curl -vk https://localhost:8412/hosts/manage') do
